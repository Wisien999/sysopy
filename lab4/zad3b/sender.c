...
int main(int argc, char** argv) {
    ...
    sigset_t suspentionMask;
    sigfillset(&suspentionMask);
    sigdelset(&suspentionMask, SIGINT);
    sigdelset(&suspentionMask, COUNT_SIGNAL);

    if (mode == KILL || mode == SIGRT) {
        for (int i = 0; i < noOfSignalsToSend; ++i) {
            kill(catcherPID, COUNT_SIGNAL);
            sigsuspend(&suspentionMask);
        }
    ...
    printf("Send %d signals and received %d confirmations\n", noOfSignalsToSend, receivedSignals);
